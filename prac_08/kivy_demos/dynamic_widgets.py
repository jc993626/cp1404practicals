...
class DynamicWidgetsApp(App):
    ...
    def create_widgets(self):
        ...
        for name in self.name_to_phone:
            ...
            temp_button.bind(on_press=self.press_entry) # video uses in_release, does not call press_entry, refers
            # set the button's background colour
            temp_button.background_color = NEW_COLOUR
            # add the button to the "entries_box" layout widget
            self.root.ids.entries_box.add_widget(temp_button) # adds child widget to that BoxLayout
